"""현재 DB에 저장된 강의들의 학과명(department)과 매핑 테스트 (파일 직접 저장)"""
import os
...
with engine.connect() as conn:
    ...
    with open('../tmp_check_final.txt', 'w', encoding='utf-8') as f:
        f.write(f"총 {len(rows)}개의 고유 학과명 발견\n\n")
        f.write(f"{'원문 학과명':<25} | {'결과':<15}\n")
        f.write("-" * 50 + "\n")
        for r in rows:
            dept = str(r[0])
            college = get_college(dept)
            f.write(f"{dept:<25} | {college}\n")
